...
lon_interval = (lonmax - lonmin)/160
lat_interval = (latmax - latmin)/170
...
for i in range(trace_length):
    ...
    if ilength > 30:
        ...
        for j in range(ilength):
            try:
                curtime = trace_merge[-1][-1][0]
            except IndexError:
                trace_merge[-1].append([trace[i][j][0][:-2],[],0])
            ...
            if trace[i][j][0][:-2] == trace_merge[-1][-1][0]:
                trace_merge[-1][-1][1].append(district)
                if j == ilength - 1:
                    if flag:
                        trace_merge[-1][-1][1] = Counter(check_district).most_common()[0][0]
                        trace_merge[-1][-1][2] = 1
                    else:
                        trace_merge[-1][-1][1] = Counter(trace_merge[-1][-1][1]).most_common()[0][0]
                        trace_merge[-1][-1][2] = 0
                    count[-1].append(trace_merge[-1][-1][2])
            else:
                if flag:
                    trace_merge[-1][-1][1] = Counter(check_district).most_common()[0][0]
                    trace_merge[-1][-1][2] = 1
                ...
                if j == ilength - 1:
                    trace_merge[-1][-1][1] = Counter(trace_merge[-1][-1][1]).most_common()[0][0]
                    trace_merge[-1][-1][2] = trace[i][j][6]
                    count[-1].append(trace_merge[-1][-1][2])

for i in range(len(trace_merge)-1,0,-1):
    if sum(count[i]) == 0:
        del trace_merge[i]
        del count[i]
